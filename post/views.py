...
def post_index(request):
    ...
    paginator = Paginator(post_lists, 2) # Show 25 contacts per page
...
def post_detail(request, postTitleSlug):
    
    post= get_object_or_404(Post, postTitleSlug=postTitleSlug)
...
def post_update(request, postTitleSlug):
    if not request.user.is_authenticated:
        raise Http404() 
    post= get_object_or_404(Post, postTitleSlug=postTitleSlug)
...
def post_delete(request, postTitleSlug):
    if not request.user.is_authenticated:
        raise Http404() 
    post= get_object_or_404(Post, postTitleSlug=postTitleSlug)
    post.delete()
